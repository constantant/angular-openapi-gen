import { InjectionToken, inject, FactoryProvider } from '@angular/core';
import { httpResource } from '@angular/common/http';
import type { paths } from '../schema.d';
import { GITHUB_BASE_URL } from '../api-base-url.token';

export const CODESPACES_DELETE_ORG_SECRET = new InjectionToken<
  (org: string, secretName: string) => ReturnType<typeof httpResource<unknown>>
>('CODESPACES_DELETE_ORG_SECRET');

export function provideCodespacesDeleteOrgSecret(): FactoryProvider {
  return {
    provide: CODESPACES_DELETE_ORG_SECRET,
    useFactory: () => {
      const base = inject(GITHUB_BASE_URL);
      return (org: string, secretName: string) =>
        httpResource<unknown>(() => ({
          url: `${base}/orgs/${org}/codespaces/secrets/${secretName}`,
          method: 'DELETE',
        }));
    },
  };
}
