...
export type UsersGetByUsernameDiscriminated =
  UsersGetByUsernamePublic | UsersGetByUsernamePrivate;
...
function _validateResponse(value: unknown): UsersGetByUsernameResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `UsersGetByUsername response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as UsersGetByUsernameResponse;
}
...
export function provideUsersGetByUsername(): FactoryProvider {
  return {
    provide: USERS_GET_BY_USERNAME,
    useFactory: () => {
