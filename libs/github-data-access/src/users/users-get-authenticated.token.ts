...
export type UsersGetAuthenticatedDiscriminated =
  UsersGetAuthenticatedPublic | UsersGetAuthenticatedPrivate;
...
function _validateResponse(value: unknown): UsersGetAuthenticatedResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `UsersGetAuthenticated response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as UsersGetAuthenticatedResponse;
}
