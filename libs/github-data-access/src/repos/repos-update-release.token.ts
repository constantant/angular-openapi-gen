...
function _validateResponse(value: unknown): ReposUpdateReleaseResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposUpdateRelease response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposUpdateReleaseResponse;
}
...
export function provideReposUpdateRelease(): FactoryProvider {
  return {
    provide: REPOS_UPDATE_RELEASE,
    useFactory: () => {
