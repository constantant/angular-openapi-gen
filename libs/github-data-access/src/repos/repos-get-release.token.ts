...
function _validateResponse(value: unknown): ReposGetReleaseResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposGetRelease response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetReleaseResponse;
}
