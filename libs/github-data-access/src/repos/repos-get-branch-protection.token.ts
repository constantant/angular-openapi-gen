...
function _validateResponse(value: unknown): ReposGetBranchProtectionResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposGetBranchProtection response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetBranchProtectionResponse;
}
