...
function _validateResponse(
  ...
  if (!_result.valid) {
    throw new Error(
      `ReposGetTeamsWithAccessToProtectedBranch response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetTeamsWithAccessToProtectedBranchResponse;
}
...
export function provideReposGetTeamsWithAccessToProtectedBranch(): FactoryProvider {
  return {
    provide: REPOS_GET_TEAMS_WITH_ACCESS_TO_PROTECTED_BRANCH,
    useFactory: () => {
