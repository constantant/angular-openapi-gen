...
function _validateResponse(value: unknown): ReposRenameBranchResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposRenameBranch response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposRenameBranchResponse;
}
...
export function provideReposRenameBranch(): FactoryProvider {
  return {
    provide: REPOS_RENAME_BRANCH,
    useFactory: () => {
