...
export type ReposAddTeamAccessRestrictionsError =
  paths['/repos/{owner}/{repo}/branches/{branch}/protection/restrictions/teams']['post']['responses']['422']['content']['application/json'];
...
function _validateResponse(
  ...
  if (!_result.valid) {
    throw new Error(
      `ReposAddTeamAccessRestrictions response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposAddTeamAccessRestrictionsResponse;
}
...
export function provideReposAddTeamAccessRestrictions(): FactoryProvider {
  return {
    provide: REPOS_ADD_TEAM_ACCESS_RESTRICTIONS,
    useFactory: () => {
