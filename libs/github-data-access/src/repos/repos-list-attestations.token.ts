...
function _validateResponse(value: unknown): ReposListAttestationsResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposListAttestations response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposListAttestationsResponse;
}
