...
function _validateResponse(value: unknown): ReposListPagesBuildsResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposListPagesBuilds response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposListPagesBuildsResponse;
}
