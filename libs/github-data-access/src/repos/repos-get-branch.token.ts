...
export type ReposGetBranchError =
  paths['/repos/{owner}/{repo}/branches/{branch}']['get']['responses']['404']['content']['application/json'];
...
function _validateResponse(value: unknown): ReposGetBranchResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposGetBranch response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetBranchResponse;
}
