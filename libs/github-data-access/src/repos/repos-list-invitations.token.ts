...
function _validateResponse(value: unknown): ReposListInvitationsResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposListInvitations response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposListInvitationsResponse;
}
...
export function provideReposListInvitations(): FactoryProvider {
  return {
    provide: REPOS_LIST_INVITATIONS,
    useFactory: () => {
