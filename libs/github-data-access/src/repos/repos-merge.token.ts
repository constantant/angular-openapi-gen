...
function _validateResponse(value: unknown): ReposMergeResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposMerge response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposMergeResponse;
}
