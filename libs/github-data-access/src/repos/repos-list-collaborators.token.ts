...
function _validateResponse(value: unknown): ReposListCollaboratorsResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposListCollaborators response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposListCollaboratorsResponse;
}
