...
function _validateResponse(value: unknown): ReposListDeploymentsResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposListDeployments response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposListDeploymentsResponse;
}
