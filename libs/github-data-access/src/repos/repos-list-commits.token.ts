...
function _validateResponse(value: unknown): ReposListCommitsResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposListCommits response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposListCommitsResponse;
}
...
export const REPOS_LIST_COMMITS = new InjectionToken<
  (
    ...
    repo: string,
    params?:
      ReposListCommitsParams | (() => ReposListCommitsParams | undefined),
