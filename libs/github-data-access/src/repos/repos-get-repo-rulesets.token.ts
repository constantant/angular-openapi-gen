...
export type ReposGetRepoRulesetsError =
  | paths['/repos/{owner}/{repo}/rulesets']['get']['responses']['404']['content']['application/json']
  | paths['/repos/{owner}/{repo}/rulesets']['get']['responses']['500']['content']['application/json'];
...
function _validateResponse(value: unknown): ReposGetRepoRulesetsResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposGetRepoRulesets response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetRepoRulesetsResponse;
}
