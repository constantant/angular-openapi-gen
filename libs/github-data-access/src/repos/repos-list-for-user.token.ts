...
function _validateResponse(value: unknown): ReposListForUserResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposListForUser response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposListForUserResponse;
}
...
export const REPOS_LIST_FOR_USER = new InjectionToken<
  (
    username: string,
    params?:
      ReposListForUserParams | (() => ReposListForUserParams | undefined),
