...
function _validateResponse(value: unknown): ReposGetLatestPagesBuildResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposGetLatestPagesBuild response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetLatestPagesBuildResponse;
}
...
export function provideReposGetLatestPagesBuild(): FactoryProvider {
  return {
    provide: REPOS_GET_LATEST_PAGES_BUILD,
    useFactory: () => {
