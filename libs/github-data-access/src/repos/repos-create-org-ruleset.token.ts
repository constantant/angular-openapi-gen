...
function _validateResponse(value: unknown): ReposCreateOrgRulesetResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposCreateOrgRuleset response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposCreateOrgRulesetResponse;
}
