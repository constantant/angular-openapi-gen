...
function _validateResponse(value: unknown): ReposListPublicResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposListPublic response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposListPublicResponse;
}
...
export function provideReposListPublic(): FactoryProvider {
  return {
    ...
    useFactory: () => {
      ...
      return (
        params?:
          ReposListPublicParams | (() => ReposListPublicParams | undefined),
      ) =>
