...
function _validateResponse(value: unknown): ReposAddCollaboratorResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposAddCollaborator response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposAddCollaboratorResponse;
}
