...
export type ReposAddUserAccessRestrictionsBody = NonNullable<
...
export type ReposAddUserAccessRestrictionsError =
  paths['/repos/{owner}/{repo}/branches/{branch}/protection/restrictions/users']['post']['responses']['422']['content']['application/json'];
...
function _validateResponse(
  ...
  if (!_result.valid) {
    throw new Error(
      `ReposAddUserAccessRestrictions response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposAddUserAccessRestrictionsResponse;
}
