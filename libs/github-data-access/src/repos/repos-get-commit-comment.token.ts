...
function _validateResponse(value: unknown): ReposGetCommitCommentResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposGetCommitComment response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetCommitCommentResponse;
}
