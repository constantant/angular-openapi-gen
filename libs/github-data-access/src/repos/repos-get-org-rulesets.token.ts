...
function _validateResponse(value: unknown): ReposGetOrgRulesetsResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposGetOrgRulesets response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetOrgRulesetsResponse;
}
...
export const REPOS_GET_ORG_RULESETS = new InjectionToken<
  (
    org: string,
    params?:
      ReposGetOrgRulesetsParams | (() => ReposGetOrgRulesetsParams | undefined),
