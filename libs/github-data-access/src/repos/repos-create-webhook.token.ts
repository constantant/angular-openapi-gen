...
function _validateResponse(value: unknown): ReposCreateWebhookResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposCreateWebhook response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposCreateWebhookResponse;
}
