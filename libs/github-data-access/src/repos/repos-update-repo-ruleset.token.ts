...
export type ReposUpdateRepoRulesetBody = NonNullable<
...
export type ReposUpdateRepoRulesetError =
  | paths['/repos/{owner}/{repo}/rulesets/{ruleset_id}']['put']['responses']['404']['content']['application/json']
  | paths['/repos/{owner}/{repo}/rulesets/{ruleset_id}']['put']['responses']['422']['content']['application/json']
  | paths['/repos/{owner}/{repo}/rulesets/{ruleset_id}']['put']['responses']['500']['content']['application/json'];

const _responseSchema: Schema = {
  title: 'Repository ruleset',
  type: 'object',
  description: 'A set of rules to apply when specified conditions are met.',
  required: ['id', 'name', 'source', 'enforcement'],
  properties: {
    id: {
      type: 'integer',
...
function _validateResponse(value: unknown): ReposUpdateRepoRulesetResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposUpdateRepoRuleset response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposUpdateRepoRulesetResponse;
}
...
export function provideReposUpdateRepoRuleset(): FactoryProvider {
  return {
    provide: REPOS_UPDATE_REPO_RULESET,
    useFactory: () => {
