...
function _validateResponse(value: unknown): ReposCreateUsingTemplateResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposCreateUsingTemplate response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposCreateUsingTemplateResponse;
}
