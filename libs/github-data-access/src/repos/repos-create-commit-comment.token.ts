...
function _validateResponse(value: unknown): ReposCreateCommitCommentResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposCreateCommitComment response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposCreateCommitCommentResponse;
}
