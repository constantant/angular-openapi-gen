...
function _validateResponse(value: unknown): ReposUpdateReleaseAssetResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposUpdateReleaseAsset response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposUpdateReleaseAssetResponse;
}
