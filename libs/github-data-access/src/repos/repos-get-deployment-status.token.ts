...
export type ReposGetDeploymentStatusError =
  paths['/repos/{owner}/{repo}/deployments/{deployment_id}/statuses/{status_id}']['get']['responses']['404']['content']['application/json'];
...
function _validateResponse(value: unknown): ReposGetDeploymentStatusResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposGetDeploymentStatus response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetDeploymentStatusResponse;
}
...
export function provideReposGetDeploymentStatus(): FactoryProvider {
  return {
    provide: REPOS_GET_DEPLOYMENT_STATUS,
    useFactory: () => {
