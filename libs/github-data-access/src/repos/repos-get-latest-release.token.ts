...
function _validateResponse(value: unknown): ReposGetLatestReleaseResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposGetLatestRelease response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetLatestReleaseResponse;
}
...
export function provideReposGetLatestRelease(): FactoryProvider {
  return {
    provide: REPOS_GET_LATEST_RELEASE,
    useFactory: () => {
