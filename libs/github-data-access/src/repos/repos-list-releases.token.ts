...
function _validateResponse(value: unknown): ReposListReleasesResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposListReleases response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposListReleasesResponse;
}
...
export const REPOS_LIST_RELEASES = new InjectionToken<
  (
    ...
    repo: string,
    params?:
      ReposListReleasesParams | (() => ReposListReleasesParams | undefined),
