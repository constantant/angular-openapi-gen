...
function _validateResponse(value: unknown): ReposListForOrgResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposListForOrg response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposListForOrgResponse;
}
  ...
  return {
    ...
    useFactory: () => {
      ...
      return (
        org: string,
        params?:
          ReposListForOrgParams | (() => ReposListForOrgParams | undefined),
      ) =>
