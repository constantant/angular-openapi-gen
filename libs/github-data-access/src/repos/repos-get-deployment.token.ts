...
export type ReposGetDeploymentError =
  paths['/repos/{owner}/{repo}/deployments/{deployment_id}']['get']['responses']['404']['content']['application/json'];
...
function _validateResponse(value: unknown): ReposGetDeploymentResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposGetDeployment response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetDeploymentResponse;
}
