...
function _validateResponse(value: unknown): ReposListForksResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposListForks response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposListForksResponse;
}
...
export function provideReposListForks(): FactoryProvider {
  return {
    provide: REPOS_LIST_FORKS,
    useFactory: () => {
      ...
      return (
        ...
        repo: string,
        params?:
          ReposListForksParams | (() => ReposListForksParams | undefined),
      ) =>
