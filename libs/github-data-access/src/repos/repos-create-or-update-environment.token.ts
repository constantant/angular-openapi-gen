...
function _validateResponse(
  ...
  if (!_result.valid) {
    throw new Error(
      `ReposCreateOrUpdateEnvironment response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposCreateOrUpdateEnvironmentResponse;
}
...
export function provideReposCreateOrUpdateEnvironment(): FactoryProvider {
  return {
    provide: REPOS_CREATE_OR_UPDATE_ENVIRONMENT,
    useFactory: () => {
