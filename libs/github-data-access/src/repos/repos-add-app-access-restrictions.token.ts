...
function _validateResponse(
  ...
  if (!_result.valid) {
    throw new Error(
      `ReposAddAppAccessRestrictions response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposAddAppAccessRestrictionsResponse;
}
...
export function provideReposAddAppAccessRestrictions(): FactoryProvider {
  return {
    provide: REPOS_ADD_APP_ACCESS_RESTRICTIONS,
    useFactory: () => {
