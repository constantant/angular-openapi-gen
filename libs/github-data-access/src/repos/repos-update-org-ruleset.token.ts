...
function _validateResponse(value: unknown): ReposUpdateOrgRulesetResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposUpdateOrgRuleset response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposUpdateOrgRulesetResponse;
}
...
export function provideReposUpdateOrgRuleset(): FactoryProvider {
  return {
    provide: REPOS_UPDATE_ORG_RULESET,
    useFactory: () => {
