...
function _validateResponse(value: unknown): ReposGetEnvironmentResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposGetEnvironment response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetEnvironmentResponse;
}
