...
function _validateResponse(value: unknown): ReposListCommentsForCommitResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposListCommentsForCommit response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposListCommentsForCommitResponse;
}
