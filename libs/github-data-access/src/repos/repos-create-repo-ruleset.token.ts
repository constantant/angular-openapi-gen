...
function _validateResponse(value: unknown): ReposCreateRepoRulesetResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposCreateRepoRuleset response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposCreateRepoRulesetResponse;
}
