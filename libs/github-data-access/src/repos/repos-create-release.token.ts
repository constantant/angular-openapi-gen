...
function _validateResponse(value: unknown): ReposCreateReleaseResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposCreateRelease response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposCreateReleaseResponse;
}
