...
export type ReposGetCodeFrequencyStatsResponse =
  ...
  | paths['/repos/{owner}/{repo}/stats/code_frequency']['get']['responses']['202']['content']['application/json'];
...
function _validateResponse(value: unknown): ReposGetCodeFrequencyStatsResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposGetCodeFrequencyStats response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetCodeFrequencyStatsResponse;
}
