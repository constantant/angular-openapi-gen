...
export type ReposGetOrgRuleSuiteError =
  | paths['/orgs/{org}/rulesets/rule-suites/{rule_suite_id}']['get']['responses']['404']['content']['application/json']
  | paths['/orgs/{org}/rulesets/rule-suites/{rule_suite_id}']['get']['responses']['500']['content']['application/json'];
...
function _validateResponse(value: unknown): ReposGetOrgRuleSuiteResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposGetOrgRuleSuite response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetOrgRuleSuiteResponse;
}
...
export function provideReposGetOrgRuleSuite(): FactoryProvider {
  return {
    provide: REPOS_GET_ORG_RULE_SUITE,
    useFactory: () => {
