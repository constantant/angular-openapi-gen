...
export type ReposGetError =
  | paths['/repos/{owner}/{repo}']['get']['responses']['403']['content']['application/json']
  | paths['/repos/{owner}/{repo}']['get']['responses']['404']['content']['application/json'];
...
function _validateResponse(value: unknown): ReposGetResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `ReposGet response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as ReposGetResponse;
}
