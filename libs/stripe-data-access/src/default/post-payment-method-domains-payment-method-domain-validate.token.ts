...
export type PostPaymentMethodDomainsPaymentMethodDomainValidateBody =
  NonNullable<
...
export function providePostPaymentMethodDomainsPaymentMethodDomainValidate(): FactoryProvider {
  return {
    provide: POST_PAYMENT_METHOD_DOMAINS_PAYMENT_METHOD_DOMAIN_VALIDATE,
    useFactory: () => {
      ...
      return (
        ...
        httpResource<PostPaymentMethodDomainsPaymentMethodDomainValidateResponse>(
          () => ({
            url: `${base}/v1/payment_method_domains/${paymentMethodDomain}/validate`,
            method: 'POST',
            body,
          }),
        );
