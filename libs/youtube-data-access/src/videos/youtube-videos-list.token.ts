...
function _validateResponse(value: unknown): YoutubeVideosListResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `YoutubeVideosList response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as YoutubeVideosListResponse;
}

export const YOUTUBE_VIDEOS_LIST = new InjectionToken<
  (
    params?:
      YoutubeVideosListParams | (() => YoutubeVideosListParams | undefined),
