...
function _validateResponse(value: unknown): YoutubeLiveStreamsUpdateResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `YoutubeLiveStreamsUpdate response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as YoutubeLiveStreamsUpdateResponse;
}
...
export function provideYoutubeLiveStreamsUpdate(): FactoryProvider {
  return {
    ...
    useFactory: () => {
      ...
      return (
        body:
          YoutubeLiveStreamsUpdateBody | Signal<YoutubeLiveStreamsUpdateBody>,
      ) =>
