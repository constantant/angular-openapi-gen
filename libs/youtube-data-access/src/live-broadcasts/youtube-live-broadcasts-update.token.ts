...
function _validateResponse(
  value: unknown,
): YoutubeLiveBroadcastsUpdateResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `YoutubeLiveBroadcastsUpdate response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as YoutubeLiveBroadcastsUpdateResponse;
}

export const YOUTUBE_LIVE_BROADCASTS_UPDATE = new InjectionToken<
  (
    body:
      YoutubeLiveBroadcastsUpdateBody | Signal<YoutubeLiveBroadcastsUpdateBody>,
