...
function _validateResponse(value: unknown): YoutubeLiveBroadcastsListResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `YoutubeLiveBroadcastsList response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as YoutubeLiveBroadcastsListResponse;
}
