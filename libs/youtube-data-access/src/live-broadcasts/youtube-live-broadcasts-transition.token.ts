...
function _validateResponse(
  value: unknown,
): YoutubeLiveBroadcastsTransitionResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `YoutubeLiveBroadcastsTransition response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as YoutubeLiveBroadcastsTransitionResponse;
}
