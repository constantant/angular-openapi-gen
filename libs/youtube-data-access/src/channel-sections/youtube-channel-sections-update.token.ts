...
function _validateResponse(
  value: unknown,
): YoutubeChannelSectionsUpdateResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `YoutubeChannelSectionsUpdate response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as YoutubeChannelSectionsUpdateResponse;
}
