...
function _validateResponse(value: unknown): YoutubePlaylistItemsInsertResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `YoutubePlaylistItemsInsert response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as YoutubePlaylistItemsInsertResponse;
}
