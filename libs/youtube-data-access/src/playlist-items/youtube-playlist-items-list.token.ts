...
function _validateResponse(value: unknown): YoutubePlaylistItemsListResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `YoutubePlaylistItemsList response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as YoutubePlaylistItemsListResponse;
}
