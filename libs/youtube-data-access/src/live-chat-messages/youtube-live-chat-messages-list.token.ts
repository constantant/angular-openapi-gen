...
function _validateResponse(
  value: unknown,
): YoutubeLiveChatMessagesListResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `YoutubeLiveChatMessagesList response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as YoutubeLiveChatMessagesListResponse;
}
