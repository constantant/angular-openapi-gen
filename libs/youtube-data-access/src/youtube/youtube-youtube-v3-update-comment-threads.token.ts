...
function _validateResponse(
  value: unknown,
): YoutubeYoutubeV3UpdateCommentThreadsResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `YoutubeYoutubeV3UpdateCommentThreads response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as YoutubeYoutubeV3UpdateCommentThreadsResponse;
}
