...
function _validateResponse(value: unknown): YoutubePlaylistsInsertResponse {
  const _result = new Validator(_responseSchema).validate(value);
  if (!_result.valid) {
    throw new Error(
      `YoutubePlaylistsInsert response failed schema validation: ${JSON.stringify(_result.errors)}`,
    );
  }
  return value as YoutubePlaylistsInsertResponse;
}
